...
language_codes = json.load(open('language_codes.json', 'r'))
...
def load_dataset(language, invert, batch_size=128, train_test_split=0.9, max_tokens=128):
  ...
  input_vectorizer = layers.TextVectorization(input_vocab_size, standardize=None, output_sequence_length=max_tokens)
  target_vectorizer = layers.TextVectorization(target_vocab_size, standardize=None, output_sequence_length=max_tokens)
